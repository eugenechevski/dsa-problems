...
class Solution:
    def carFleet(self, target: int, position: list, speed: list) -> int:
        ...
        for i in range(target, -1, -1):
            if road[i] > 0:
                catches_up = False
                if stack and road[stack[-1]] < road[i]:
                    meeting_time = (stack[-1] - i) / \
                        (road[i] - road[stack[-1]])
                    meeting_position = stack[-1] + \
                        road[stack[-1]] * meeting_time
                    completion_time = ((target - stack[-1]) / road[stack[-1]])
                    if meeting_position <= target or \
                       (i + road[i] * completion_time) >= (stack[-1] + road[stack[-1]] * completion_time):
                        fleet -= 1
                        catches_up = True
