...
class Solution:
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        original_q = deque([node])  # original queue for iteration
        copy_q = deque([Node(val=node.val)])  # copy queue for iteration
        root_copy = copy_q[0]
        ...
        # BFS
        while original_q:
            curr_original = original_q.popleft()
            curr_copy = copy_q.popleft()

            for neighbor in curr_original.neighbors:
                if neighbor.val not in visited:
                    # Create a new copy
                    original_q.append(neighbor)
                    curr_copy.neighbors.append(Node(val=neighbor.val))
                    copy_q.append(curr_copy.neighbors[-1])
                    visited[copy_q[-1].val] = copy_q[-1]
                else:
                    # Use the existing copy
                    curr_copy.neighbors.append(visited[neighbor.val])
